import React, { useState, useCallback, useLayoutEffect, useContext, useMemo, useEffect } from 'react';
...
const useClapAnimation = ({ clapRef, clapCountRef, clapCountTotalRef }) => {
    ...
    const [animationTimeline, updateAnimationTimeline] = useState(() =>
        new mojs.Timeline()
    );

    useLayoutEffect(() => {
        if (clapRef && clapCountRef && clapCountTotalRef) {
            ...
            updateAnimationTimeline((currentState) =>
                currentState.add([scaleButton, countTotalAnimation, countAnimation, triangleBurst, circleBurst])
            );

        }

    }, [clapRef, clapCountRef, clapCountTotalRef]);
...
const MediumClap = ({ children, onClap }) => {
    ...
    const animationTimeline = useClapAnimation(refState);

    useEffect(() => {
        if(onClap && isClicked){
            onClap(clapState);
            console.log('calling onClap in useEffect');
        }
    }, [count]);
    ...
    const providerValue = useMemo(() => ({ ...clapState, setRef }), [clapState, setRef]);
    return <MediumClapContext.Provider value={providerValue}>
...
const CountTotal = () => {
    ...
    return <span ref={setRef} data-refkey="clapCountTotalRef" className={styles.total}>
        {countTotal}
    </span>
}
...
const Usage = () => {
    ...
    const handleClap = useCallback((clapState) => {
    ...
    return (
        <div style={{width: '100%'}}>
            ...
            <div>{`clap count is ${clapCount}`}</div>
        </div>
    );
}
