import React, {
  ...
  import userClassNames from './usage.css'
  ...
  const ClapIcon = ({className}) => {
    const { isClicked } = useContext(MediumClapContext);
    const classNames = useMemo(() => ([styles.icon,isClicked ? styles.checked: '', className].join(' ').trim()), [className]);
  ...
  const ClapCount = ({style: userStyles, className}) => {
    ...
    const classNames = useMemo(() => ([styles.count, className].join(' ').trim()), [className]);
    return (
  ...
  const CountTotal = ({style: userStyles, className}) => {
    ...
    const classNames = useMemo(() => ([styles.total, className].join(' ').trim()), [className]);
  ...
  const Usage = () => {
    ...
    return (
      <div style={{ width: '100%' }}>
        <MediumClap onClap={handleClap} style={{border: '1px solid red'}} className={userClassNames.clap}>
          <MediumClap.Icon className={userClassNames.icon}/>
          <MediumClap.Count style={{background: '#8cacea'}} className={userClassNames.count}/>
          <MediumClap.Total style={{background: '#8cacea', color: 'black'}} className={userClassNames.total}/>
        </MediumClap>
        {!!count && (
