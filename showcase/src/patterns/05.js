...
const Usage = () => {
    ...
    const handleClap = useCallback(() => {
        ...
        setValues(clapState => ({
            isClicked: true,
            count: Math.min(clapState.count + 1, MAXIMUM_USER_CLAP),
            countTotal:
            clapState.count < MAXIMUM_USER_CLAP
                    ? clapState.countTotal + 1
                    : clapState.countTotal
        }));
    }, []);

    return (
        <div style={{ width: '100%' }}>
            <MediumClap values={values} onClap={handleClap} className={userCustomStyles.clap}>
                ...
                <div className={styles.info}>{`You have clapped ${values.count} times`}</div>
        </div>
    )
}
